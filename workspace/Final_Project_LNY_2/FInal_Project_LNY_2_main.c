...
/*
 * Function used to determine if the segbot fell in either direction
 * Return 0 if it has not fallen, returns 1 if it fell forward, returns -1 if fell backwards
 */
int16 checkTilt()
{
...
//################################################################################
void setEPWM8A_RCServo(float angle) { //NAL: Accepts a float angle between -90 and 90 to determine the duty cycle (hence angle) of the RC servos
    ...
    EPwm8Regs.CMPA.bit.CMPA = (int16_t) (dutyCyc/100 * EPwm8Regs.TBPRD); // NAL: determine the needed value of CMPA
}

void setEPWM8B_RCServo(float angle){//NAL: Accepts a float angle between -90 and 90 to determine the duty cycle (hence angle) of the RC servos
    if (angle > 90) {//NAL: Saturate input to 90 if above 90
        angle = 90;
    } else if (angle < -90){//NAL: Saturate input to -90 if below -90
        angle=-90;
...
void main(void)
{
    ...
    // NAL: ========= FOR 8
    EPwm8Regs.TBCTL.bit.CTRMODE = 0;//NAL: Count up Mode
    EPwm8Regs.TBCTL.bit.FREE_SOFT = 3; // 1x is 10 or 11 so 3 or 4 //NAL: Free Soft emulation mode to Free run
    EPwm8Regs.TBCTL.bit.PHSEN = 0;
    EPwm8Regs.TBCTL.bit.CLKDIV = 4; //NAL: set clock divide equal to 16 so that TBPRD can be set to a number that can fit into the 16 bit allowed for TBPDR

    EPwm8Regs.TBCTR = 0;//NAL: start timer at 0

    EPwm8Regs.TBPRD = 62500; //NAL: Set the period so that EPWM8 has the required RC Servo carrier frequency of 50Hz
    ...
    EPwm8Regs.TBPHS.bit.TBPHS = 0;  //NAL: set the phase to 0
    ...
    //NAL: For 8B
    EPwm8Regs.CMPB.bit.CMPB = 0; //NAL: start duty cycle at 0%
    EPwm8Regs.AQCTLB.bit.CBU = 1;//NAL:  have the signal pin be cleared when the TBCTR register reaches the value in CMPB
...
//    }
//
//    if (increaseOrDecreaseMotors) {//NAL: increase controleffort by .005 to increase motor angle gradually
//        currAngle = currAngle + .05;
//
//    } else{//NAL: decrease controleffort by .005 to decrease motor angle gradually
//        currAngle = currAngle - .05;
//    }
